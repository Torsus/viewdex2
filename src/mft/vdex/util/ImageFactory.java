...
public class ImageFactory {
    ...
    public static BufferedImage createGrayscaleImage(int imageWidth, int imageHeight,
    int imageDepth, short data[]){
        ...
        imageHeight,
        1,               // pixel stride
        imageWidth,      // scanline stride
        new int[] {0});  // bandOffsets
        
        DataBuffer dataBuf = new DataBufferUShort(
        (short[])data,
        imageWidth);
        ...
        for(int i=0; i<ht; i++){
            for(int j=0; j<wid; j++){
                for(int k=0;k<numBands;k++) {
                    if(k==0 && hasAlpha) data1[((i*wid+j)*numBands)+k] = (short)(255);
                    else data1[((i*wid+j)*numBands)+k] = (short)(255* Math.abs(Math.random()));
                }
            }
        }
    ...
    public static BufferedImage createGrayscaleBar(int wid, int ht, int maxvalue){
        ...
        float ratio = (float)((float)maxvalue/(float)ht);
        for(int i=0; i<ht; i++){
            for(int j=0; j<wid; j++){
                data[i*wid+j] = (short)(i*ratio);
                //short a = data[i*wid+j];
                //short b = 10;
            }
        }
    ...
    // Cat
    public BufferedImage createCatGrayscaleImage(int wid, int ht, int bits){
        short data[] = new short[wid*ht];
        return createGrayscaleImage(wid,ht,bits,data);
    }
    ...
    public static void  main(String[] args){
        ...
        BufferedImage bi1 = gs.createBandedImage(64,64,8,data1);
        //if(bi1 != null)
          //  JpegUtil.saveImageAsJPEG(bi1, "rgb.jpg");
    }
    
    // test
    public static BufferedImage createGrayscaleImage2(int imageWidth, int imageHeight,
    int imageDepth, short data[]){
        ComponentColorModel ccm = new ComponentColorModel(
        ColorSpace.getInstance(ColorSpace.CS_GRAY),
        new int[] {imageDepth},
